...
class DiffMatchPatch
{
	...
	public function diffMain($text1, $text2, $checkLines = true)
	{
		...
		if ($text1 === $text2) {
			return $text1 ? array(array(self::DIFF_EQUAL, $text1)) : array();
		}
	...
	protected function diffCompute($text1, $text2, $checkLines)
	{
		if ($text1 === '') {
			...
			return $text2 === '' ? array() : array(array(self::DIFF_INSERT, $text2));
		}
		...
		if ($i !== false) {
			...
			if (!mb_strlen($diffs[2][1])) {
				array_pop($diffs);
			}

			if (!mb_strlen($diffs[0][1])) {
				array_shift($diffs);
			}
		...
		if (mb_strlen($shortText) == 1) {
			return array(array(self::DIFF_DELETE, $text1), array(self::DIFF_INSERT, $text2));
		}
	...
	protected function diffMap($text1, $text2)
	{
		...
		for ($d = 0; $d < $max_d; $d++) {
			...
			for ($k = -$d; $k <= $d; $k += 2) {
				...
				while (!$done && ($x < $text1length) && ($y < $text2length) && (mb_substr($text1, $x,
							1) == mb_substr($text2, $y, 1))) {
					$x++;
					$y++;
			...
			if ($doubleEnd) {
				...
				for ($k = -$d; $k <= $d; $k += 2) {
					...
					while (!$done && $x < $text1length && $y < $text2length && mb_substr($text1, $text1length - $x - 1,
							1) == mb_substr($text2, $text2length - $y - 1, 1)) {
						$x++;
						$y++;
						$footstep = ($text1length - $x) . ',' . ($text2length - $y);
					...
					if ($done) {
						...
						$a = $this->diffPath1($v_map1, mb_substr($text1, 0, $text1length - $x),
							mb_substr($text2, 0, $text2length - $y));

						return array_merge($a, $this->diffPath2($v_map2, mb_substr($text1, $text1length - $x),
							mb_substr($text2, $text2length - $y)));
					}
				}
			}
		}
	...
	protected function diffHalfMatchI($longText, $shortText, $i)
	{
		...
		while (($j = mb_strpos($shortText, $seed, $j + 1)) !== false) {
			...
			if (mb_strlen($bestCommon) < $suffixLength + $prefixLength) {
				$bestCommon = mb_substr($shortText, $j - $suffixLength, $suffixLength) . mb_substr($shortText, $j,
						$prefixLength);
	...
	protected function diffCleanupSemantic(&$diffs)
	{
		...
		while ($pointer < count($diffs)) {
			if ($diffs[$pointer][0] == self::DIFF_EQUAL) {
				...
				{ //+
					$lengthInsertions2 += mb_strlen($diffs[$pointer][1]); //+
				} //+
				else //+
				{ //+
					$lengthDeletions2 += mb_strlen($diffs[$pointer][1]); //+
				} //+

				//if ($lastEquality !== null && (mb_strlen($lastEquality) <= $lengthChanges1) && (mb_strlen($lastEquality) <= $lengthChanges2))
				if ($lastEquality && (mb_strlen($lastEquality) <= max($lengthInsertions1,
							$lengthDeletions1)) && (mb_strlen($lastEquality) <= max($lengthInsertions2,
							$lengthDeletions2))) //+
				{
					// Duplicate record
		...
		while ($pointer < count($diffs)) {
			if ($diffs[$pointer - 1][0] == self::DIFF_DELETE && $diffs[$pointer][0] == self::DIFF_INSERT) {
				...
				if ($overlapLength1 >= $overlapLength2) {
					if ($overlapLength1 >= mb_strlen($deletion) / 2 || $overlapLength1 >= mb_strlen($insertion) / 2) {
						// Overlap found.  Insert an equality and trim the surrounding edits.
						array_splice($diffs, $pointer, 0,
							array(self::DIFF_EQUAL, mb_substr($insertion, 0, $overlapLength1)));
						$diffs[$pointer - 1][1] = mb_substr($deletion, 0, mb_strlen($deletion) - $overlapLength1);
						$diffs[$pointer + 1][1] = mb_substr($insertion, $overlapLength1);
						$pointer++;
					}
				} else {
					if ($overlapLength2 >= mb_strlen($deletion) / 2 || $overlapLength2 >= mb_strlen($insertion) / 2) {
						// Reverse overlap found.
						// Insert an equality and swap and trim the surrounding edits.
						array_splice($diffs, $pointer, 0,
							array(self::DIFF_EQUAL, mb_substr($deletion, 0, $overlapLength2)));
						$diffs[$pointer - 1][0] = self::DIFF_INSERT;
						$diffs[$pointer - 1][1] = mb_substr($insertion, 0, mb_strlen($insertion) - $overlapLength2);
						$diffs[$pointer + 1][0] = self::DIFF_DELETE;
						$diffs[$pointer + 1][1] = mb_substr($deletion, $overlapLength2);
						$pointer++;
					}
				}
	 ...
	 * @param string $text1 First string.
	 * @param string $text2 Second string.
	 * @return integer The number of characters common to the end of the first
	 *     string and the start of the second string.
	 */
	protected function diffCommonOverlap($text1, $text2)
	{
		// Cache the text lengths to prevent multiple calls.
		$text1Length = mb_strlen($text1);
		$text2Length = mb_strlen($text2);
		...
		if ($text1Length > $text2Length) {
			$text1 = mb_substr($text1, $text1Length - $text2Length);
		} elseif ($text1Length < $text2Length) {
			$text2 = mb_substr($text2, 0, $text1Length);
		}
		...
		while (true) {
			$pattern = mb_substr($text1, $textLength - $length);
			...
			if ($found == 0 || mb_substr($text1, $textLength - $length) == mb_substr($text2, 0, $length)) {
				$best = $length;
				$length++;
			}
		}
	...
	protected function diffCleanupSemanticLossless(&$diffs)
	{
		...
		while ($pointer < count($diffs) - 1) {
			if ($diffs[$pointer - 1][0] == self::DIFF_EQUAL && $diffs[$pointer + 1][0] == self::DIFF_EQUAL) {
				...
				$bestScore = $this->diffCleanupSemanticScore($equality1, $edit) + $this->diffCleanupSemanticScore($edit,
						$equality2);

				while (mb_strlen($equality2) && ($editChar = mb_substr($edit, 0,
						1)) === ($equality2char = mb_substr($equality2[0], 0, 1))) {
					$equality1 .= $editChar;
					$edit = mb_substr($edit, 1) . $equality2char;
					$equality2 = mb_substr($equality2, 1);
					$score = $this->diffCleanupSemanticScore($equality1, $edit) + $this->diffCleanupSemanticScore($edit,
							$equality2);
	...
	/**
	 * Reorder and getMergedElement like edit sections.  Merge equalities.
	...
	protected function diffCleanupMerge(&$diffs)
	{
			...
			switch ($diffs[$pointer][0]) {
				...
				case self::DIFF_EQUAL :
					...
					if ($count_delete !== 0 || $count_insert !== 0) {
						if ($count_delete !== 0 && $count_insert !== 0) {
							...
							if ($commonLength !== 0) {
								if (($pointer - $count_delete - $count_insert) > 0 && $diffs[$pointer - $count_delete - $count_insert - 1][0] == self::DIFF_EQUAL) {
									$diffs[$pointer - $count_delete - $count_insert - 1][1] .= mb_substr($text_insert,
										0, $commonLength);
								} else {
							...
							if ($commonLength !== 0) {
								$diffs[$pointer][1] = mb_substr($text_insert,
										mb_strlen($text_insert) - $commonLength) . $diffs[$pointer][1];
						...
						if ($count_delete === 0) {
							array_splice($diffs, $pointer - $count_delete - $count_insert,
								$count_delete + $count_insert, array(
		...
		while ($pointer < count($diffs) - 1) {
			if ($diffs[$pointer - 1][0] == self::DIFF_EQUAL && $diffs[$pointer + 1][0] == self::DIFF_EQUAL) {
				// This is a single edit surrounded by equalities.
				if (mb_substr($diffs[$pointer][1],
						mb_strlen($diffs[$pointer][1]) - mb_strlen($diffs[$pointer - 1][1])) == $diffs[$pointer - 1][1]) {
					// Shift the edit over the previous equality.
					$diffs[$pointer][1] = $diffs[$pointer - 1][1] . mb_substr($diffs[$pointer][1], 0,
							mb_strlen($diffs[$pointer][1]) - mb_strlen($diffs[$pointer - 1][1]));
					$diffs[$pointer + 1][1] = $diffs[$pointer - 1][1] . $diffs[$pointer + 1][1];
					array_splice($diffs, $pointer - 1, 1);
					$changes = true;
				} elseif (mb_substr($diffs[$pointer][1], 0,
						mb_strlen($diffs[$pointer + 1][1])) == $diffs[$pointer + 1][1]) {
					// Shift the edit over the next equality.
					$diffs[$pointer - 1][1] .= $diffs[$pointer + 1][1];

					$diffs[$pointer][1] = mb_substr($diffs[$pointer][1],
							mb_strlen($diffs[$pointer + 1][1])) . $diffs[$pointer + 1][1];
					array_splice($diffs, $pointer + 1, 1);
					$changes = true;
				}
	...
	public function diffXIndex($diffs, $loc)
	{
		...
		for ($x = 0, $l = count($diffs); $x < $l; $x++) {
			if ($diffs[$x][0] !== self::DIFF_INSERT) { // Equality or deletion.
				$chars1 += mb_strlen($diffs[$x][1]);
			}

			if ($diffs[$x][0] !== self::DIFF_DELETE) { // Equality or insertion.
	 ...
	 *
	 * @param DiffPatchObject $patch The patch to grow.
	...
	protected function patchAddContext($patch, $text)
	{
		...
		while ((mb_strlen($pattern) === 0 // Javascript's indexOf/lastIndexOf return 0/strlen respectively if pattern = ''
				|| mb_strpos($text, $pattern) !== mb_strrpos($text,
					$pattern)) && $pattern !== $previousPattern // avoid infinite loop
			&& mb_strlen($pattern) < $this->MATCH_MAX_BITS - $this->Patch_Margin - $this->Patch_Margin) {
			$padding += $this->Patch_Margin;
			$previousPattern = $pattern;
			$pattern = mb_substr($text, max($patch->start2 - $padding, 0),
				($patch->start2 + $patch->length1 + $padding) - max($patch->start2 - $padding, 0));
		}

		// Add one chunk for good luck.
		$padding += $this->Patch_Margin;
		// Add the prefix.
		$prefix = mb_substr($text, max($patch->start2 - $padding, 0),
			$patch->start2 - max($patch->start2 - $padding, 0));
		...
		// Add the suffix.
		$suffix = mb_substr($text, $patch->start2 + $patch->length1,
			($patch->start2 + $patch->length1 + $padding) - ($patch->start2 + $patch->length1));
	 ...
	 * @return DiffPatchObject[] {Array.<patch_obj>} Array of patch objects.
		...
		for ($x = 0, $l = count($diffs); $x < $l; $x++) {
			...
			switch ($diffType) {
				case self::DIFF_INSERT :
					...
					$postPatchText = mb_substr($postPatchText, 0, $charCount2) . $diffText . mb_substr($postPatchText,
							$charCount2);
					break;
				...
				case self::DIFF_DELETE :
					...
					$patch->diffs[$patchDiffLength++] = $diffs[$x];
					$postPatchText = mb_substr($postPatchText, 0, $charCount2) . mb_substr($postPatchText,
	 ...
	 *
	 * @param DiffPatchObject[] $patches {Array.<patch_obj>} Array of patch objects.
	 * @return DiffPatchObject[] {Array.<patch_obj>} Array of patch objects.
	 */
	 ...
	 * @return array {Array.<string|Array.<boolean>>} Two element Array, containing the
	...
	public function patchApply($patches, $text)
	{
		...
		for ($x = 0; $x < count($patches); $x++) {
			...
			if ($text1length > $this->MATCH_MAX_BITS) {
				...
				if ($startLoc != -1) {
					$endLoc = $this->matchMain($text, mb_substr($text1, $text1length - $this->MATCH_MAX_BITS),
						$expectedLoc + $text1length - $this->MATCH_MAX_BITS);
			...
			if ($startLoc == -1) {
				...
				if ($text1 == $text2) {
					...
					$text = mb_substr($text, 0, $startLoc) . $this->diffText2($patches[$x]->diffs) . mb_substr($text,
							$startLoc + $text1length);
				} else {
					...
					if ($text1length > $this->MATCH_MAX_BITS && $this->diffLevenshtein($diffs) / $text1length > $this->Patch_DeleteThreshold) {
						...
						for ($y = 0; $y < count($patches[$x]->diffs); $y++) {
							...
							if ($mod[0] === self::DIFF_INSERT) {
								// Insertion
								$text = mb_substr($text, 0, $startLoc + $index2) . $mod[1] . mb_substr($text,
										$startLoc + $index2);
							} elseif ($mod[0] === self::DIFF_DELETE) {
								// Deletion
								$text = mb_substr($text, 0, $startLoc + $index2) . mb_substr($text,
										$startLoc + $this->diffXIndex($diffs, $index1 + mb_strlen($mod[1])));
							}
